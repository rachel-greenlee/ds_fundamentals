...
### Linear Models v Generalized Linear Models v Genralized Least Squares Model

The *lm* function in R is for linear regression models, where:  
1) normal errors  
2) constant variance  

The *glm* function in R is for generalized linear models, where:  
1) non-normal errors  
2) non-constant variance  
The slope can be interpreted as the change in the response variable for 1 unit of change in the independent variable, if you do e to the power of that value. #exp(coef(model1)[2]) will give you that value  



The *gls* function in R is for generalized least squares models, where:  
1) correlated errors  
2) spatial/temporal patterns/trends

30-minute video explaining differences and fitting a model from each to the same data:
https://www.youtube.com/watch?v=P-WYkSZp9lY

#### Consider Robust Regression

For some models there doesn't need to be constant variance (such as in Poission regressions), and it can be useful to build that varying variance into the model. You can do so with Robust Regression. This does *not* apply to models that have a required assumption of constant variance, such as linear regression.

Robust regression is an alternative to least squares regression when data are contaminated with outliers or influential observations, and it can also be used for the purpose of detecting influential observations. The idea of robust regression is to weigh the observations differently based on how well behaved these observations are.

Weighted-Least-Squares (WLS) Estimation is used when variance is not constant, which is a requirement of Ordinary-Leasty Squares (OLS) Estimation. When WLS makes a model, it recognizes areas along the x-axis that have higher variance, and then adjusts the line/model accordingly. So an outlier in the ‘low variance’ section of the x-axis would have more impact on adjusting the model than an outlier in the ‘high variance’ section of the x-axis (where we expect more variance).

Graphic intuition about WLS vs OLS starting minute 3: https://www.youtube.com/watch?v=wCJ8I-MtJdQ&ab_channel=BenLambert

Very succinct walk-through of checking if WLS is needed and implementing it, then comparing the two models:
https://www.statology.org/weighted-least-squares-in-r/  

#### Check Autocorrelation (Serially Correlated) Errors for Time Series Data  

Time series data often follows a pattern, and that pattern can affect the errors and prediction of a model. In R we can check for this 'lag', the affect the previous time period has on the current time period, and then incorporate it into the model for a better fit.

Great datacamp tutorial page with how to check for autocorrelation by various lags:
https://www.datacamp.com/community/tutorials/autocorrelation-r

Simple example on Rpubs:
https://rpubs.com/fractalbass/615815


### Multiple/Linear Regression

See fantastic case study on page 147 of Practical Machine Learning in R book I own (with first half of chapter explaining all related concepts).

Regression techniques are a category of ML algorithms that seek to predict a numeric response by quantifying the size and strength of the relationship between numerical values.

Used when response variable is quantitative and continuous. When you have only 1 independent variable it's called a *simple linear regression*, with more than 1 independent variable it is *multiple linear regression*.
...
*Correlation* is  statistic term used to describe and quantify the relationship between two variables. The *correlation coefficient* is the value that quantifies this relationship, with Pearson's correlation coefficient being the most commonly used for linear relationships (with larger absolute values indicating stronger relationships, whether positive or negative).  

Ordinary Least Squares (OLS) method is the simplest way to find the regression line. Essentially it minimizes the residuals, the errors, when choosing a line to shoot through the datapoints. The line's predicted values for all x's is subtracted from the actual value in the dataset, and it's absolute value is summed up with the rest. This is the *residual sum of squares* or *sum of squared errors*.

Example of multiple linear regression code from Wine Sales project.
...
The summary output has multiple sections.  
1. The residuals section shows the minimum and maximum residual found with the produced model, while the median residual shows a sense of the typical model performance.   
2. The coefficients section can be helpful for model interpretability.  
3. The standard error, which is the standard deviation of the parameter estimates is included. The lower the standard error is with regard to the estimate, the better the estimate is. On the p-values, three *** means that variable is significant at a value less than 0.001 - you'll see the legend below the output of the other *** levels.  
4. The residual standard error is the average amount that the predicted response will deviate from the observed data. This is a measure of lack of fit for a model. Whether the number is good or bad is dependent on the context of the problem. In general, the smaller the RSE, the more confident we are that our model fits our data well. The degrees of freedom value provides the number of data point sin our model that are variable.  
5. The R-squared statistic provides an alternative measure of fit from the RSE. It provides a measure of fit independent of the scale of Y and is a proportion with values between 0 and 1. It allows us to say, "this model accounts for 63.9% of the variance in the data." The adjusted R-squared is slightly modified to penalize models with a large number of independent variables. This is useful when comparing models to each other.  
6. The F-statistic is a statistical test of whether there exists a relationships between the predictor and the response variables. The larger the value for the F-statistic, the stronger the relationship. The p-value here is more useful, it tells us how likely it is that the variables in our regression model fit the data in a statistically significant manner. The closer the value is to zero, the better the fit. 
...
Used for rates, proportions, contingency tables. If large enough (such as a mean greater than 30) it will approach normal so you probably don’t need to do a Poisson - but often there isn’t enough data so a Poisson Model is needed. Further, the data of counts is always positive. The mean and variance of a Poisson distribution equals lambda. You use log-scale coefficients.

If the residual deviance is much larger than the residual degrees of freedom, then the data is overdispersed - as we expect a model that fits reasonably well to have a residual deviance roughly equal to the degrees of freedom. John Fox argues overdispersion is so common in regression models for count data, and its consequences are potentially so severe, that models such as the quasi-Poisson and negative-binomial GLMs discussed in this section should be employed as a matter of course in preference to the Poisson GLM (Applied Regression Analysis & Generalized Linear Models). 

The Poisson distribution has only one parameter and so it’s not very flexible for empirical fitting purposes. One can add a dispersion parameter (making it a quasi-Poisson) or model the response as a negative binomial or other more flexible distribution if the specific mechanism of the data relationship is known. If data has a variance that is over the mean, your data is over-dispersed so you need something other than a Poisson. Zero inflation is when there are far more zero values in response variable.

If the count is some number out of a possible total/ound, then the response might be better modeled as a binomial. However, for small success probabilities (low rates) and large totals (n) the Poisson is a good approximation and can be applied. For example, rates of a cancer across a state will have a large n and a small p. 

If individual count data meets Poisson, so will the sum of those counts aka aggregate data. This can be useful if you only have the sum counts.

The Poisson deviance is also known as the G-statistic.

See 'Consider Robust Regression' section if variance if data is not constant.

4 minute intro to why/when Poisson is needed: https://www.youtube.com/watch?v=xgifo6nFo1s&ab_channel=DataCamp

R walkthrough of Poission: https://stats.idre.ucla.edu/r/dae/poisson-regression/
...
Example of code for modeling taken from Wine Sales project. 
...
# 
# #comparing models to each other with Vuong Non-Nested Hypothesis Test-Statistic
...
Code example from Wine Sales project.
...
### Binomil Model & Multinomial Logit Model

Use a binomial model when the response has two values, if it has more use a multinomial model.  

There is a distinction between *nominal* multinomial data, where there is no natural order to the categories, and *ordinal* multinomial data, where this is an order. **The multinomial logit model is intended for nominal data. It can be used for ordinal data, but the information about order will not be incorporated into the model.**

Example in R  

Part 1 - Data & Model (6 minutes)
https://www.youtube.com/watch?v=S2rZp4L_nXo&ab_channel=Dr.BharatendraRai


Part 2 - Final Model & Interpretation (8 minutes)
https://www.youtube.com/watch?v=oxRy2DMrOF4&ab_channel=Dr.BharatendraRai
...
## Creating predicted values to check accuracy

One can use a handful of different models and use the `predict()` function to create guesses of the target variable off of the test/hold-out data set. Then one can compare the predicted target variable with the actual found in the test/hold-out data set.

Code example from Wine Sales project. This outputs simply a percentage representating the accuracy for each model tested.
